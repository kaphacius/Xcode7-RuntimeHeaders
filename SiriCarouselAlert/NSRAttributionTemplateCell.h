//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//


@interface NSRAttributionTemplateCell : NSRTemplateCell
{
    NSRImageView *_attributionImageView;
    NSRLabel *_attributionLabel;
}

+ (float)desiredBaselineForCellBelowLayoutType:(int)arg1;
+ (float)firstLineBaselineOriginWithTemplate:(id)arg1;
+ (int)defaultContentAlignmentType;
+ (int)defaultLayoutType;
+ (float)_attributionLabelBaselineToBottom;
+ (_Bool)wantsPlatterBackground:(id)arg1;
+ (id)calculateLayoutValuesForTemplate:(id)arg1 suggestedSize:(struct CGSize)arg2;
- (void).cxx_destruct;
- (id)_layoutValues;
- (void)setTemplateObject:(id)arg1;
- (void)layoutSubviews;
- (id)initWithStyle:(int)arg1 reuseIdentifier:(id)arg2;

@end